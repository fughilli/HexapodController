...
class WorldLimb(object):
    ...
    def _lin_callback(pos):
        self.motion_callback(lut(*pos))
...
class SCInputController(object):
    ...
    def _axis_control_hook(x, y, pressed, world_limbs):
    ...
    def _control_hook(lx, ly, lp, rx, ry, rp):
        if not self._update_flag:
            return
        self._update_flag = False
        _axis_control_hook(lx, ly, lp, self.l_world_limbs)
        _axis_control_hook(rx, ry, rp, self.r_world_limbs)

    def _steam_controller_hook(_, data):
...
leg_origin = (55, 0, -50)
leg0_angle = 2 * pi / 12
leg_angles = [2 * pi / num_legs * i + leg0_angle for i in range(num_legs)]
...
scic = SCInputController(0.1, world_limbs, [0, 2, 4], [1, 3, 5], 25, 35, 0)
